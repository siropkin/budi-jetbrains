...
- JetBrains Marketplace: search for "budi" in `Settings → Plugins → Marketplace`, or jump to the listing directly at [`plugins.jetbrains.com/plugin/31662-budi`](https://plugins.jetbrains.com/plugin/31662-budi). Beta-channel pre-releases are installable via the custom-channel URL (see README).
...
5. **Settings panel** — `Settings → Tools → budi` exposes daemon URL, cloud endpoint, polling interval, the `includeOtherSurfaces` opt-out, the upgrade-prompt suppress toggle, and a read-only "Detected sources" row sourced from `GET /health/sources?surface=jetbrains` (degrades quietly on older daemons).
...
- **Provider parser scope (v0.1).** GitHub Copilot for JetBrains parser is daemon-side work tracked in [`siropkin/budi#722`](https://github.com/siropkin/budi/issues/722) — until it lands, `surface=jetbrains` rows in `budi stats surfaces` will read `$0.00` even with active sessions. JetBrains AI Assistant (Anthropic-backed, separate JetBrains subscription, different storage layout and wire format) is explicitly deferred to v0.2 ([#32](https://github.com/siropkin/budi-jetbrains/issues/32)). The read-only "Detected sources" row in the settings panel ([#33](https://github.com/siropkin/budi-jetbrains/issues/33)) shipped in 0.1.1 against the daemon's `GET /health/sources?surface=jetbrains` endpoint ([`siropkin/budi#735`](https://github.com/siropkin/budi/issues/735)); it degrades quietly to "No sources detected" on older daemons. Plugin stays presentation-only; do not parse JetBrains chat storage from this repo.
...
- **Test scope, no coverage gate.** Tests target the wire boundary with the daemon (request shapes, response parsing, formatting, URL allowlists, the upgrade-decision matrix, install commands, settings round-trip) and the pure listener / state-derivation surface on `BudiAppState`. UI / IDE-integration paths (`BudiConfigurable.apply()`, `BudiStatusBarWidgetFactory`, the notifier balloons) are deliberately not unit-tested — they are mostly platform-DSL glue where a Kover threshold would either reward shallow tests that mock platform APIs back at themselves, or require heavyweight `runIdeForUiTests` runs that hurt iteration speed. Plugin Verifier already gates platform-correctness regressions in CI. Invest the test budget where breakage surfaces to users (the daemon contract), not on a number.
